...
\section{Pour la résolution du système linéaire}
  ...
  \subsection{Formulation sans matrice}
    ...
    \subsubsection{Calcul de la jacobienne}
      ...
      On peut calculer une coloration de la jacobienne car on peut en général obtenir son pattern de creusité facilement puisqu'on connaît le schéma de discrétisation du code.\footnote{\LM{ok en structuré, mais dans le cas général, c'est toujours aussi simple?}}
  ...
  \subsection{Différenciation Automatique}
    ...
    \footnote{\PS{Parler d'ELSA qui s'y met ? Si oui j'espère pouvoir trouver un doc où ils comparent les perfo avec/sans AD.} \GP{Oui, en parler ! Il faut être exhaustif !}\PS{TODO}}
...
\section{Pour l'intégration temporelle}
  ...
  \subsection{Méthode de Newton pour la multiphysique}
    ...
    \end{aligned}\right.\]
    puis :\footnote{\LM{c'est bien $b\left(W_{1,k},W_{1,k} \right)$ et non pas $b\left( W_{1,k},W_{2,k}\right)$? Même chose pour $c\left( W_{1 \ast}, W_{1\ast}\right)$}}
    \[\left\{\begin{aligned}
