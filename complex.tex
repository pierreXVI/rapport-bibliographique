...
\section{Pour la résolution du système linéaire}
  ...
  \subsection{Formulation sans matrice}
    ...
    \subsubsection{Calcul de la jacobienne}
      ...
      \paragraph{}
      On peut calculer la jacobienne à partir de $F$\GP{autour de $W_O$ et dans la direction $e_i$, 
      en utilisant la dérivée de Fréchet. Sans perdre de généralité, l'approximation utilisée peut être apparentée à 
      un calcul de dérivée par la méthode des différences finies.}%des différences finies.
  ...
  \subsection{Différenciation Automatique}
    ...
    La précision obtenue est alors bien meilleure qu'avec des différences finies. \GP{Toutefois, cette approche 
    doit être traitée avec attention car de nombreuses fonctions (min ou max par exemple) ne sont pas différentiables.}
    ...
    \footnote{\PS{Parler d'ELSA qui s'y met, ou alors on les aime pas donc on en parle pas ? Si oui je pense pouvoir trouver un doc où ils comparent les perfo avec/sans AD.} \GP{Oui, en parler ! Il faut être exhaustif !}}
...
\section{Pour l'intégration temporelle}
  ...
  \subsection{Méthodes exponentielles}
    ...
    Ces méthdoes d'intégration sont depuis longtemps utilisée pour les problèmes qui concernent ma thèse, comme pour la résolution des équations de Navier-Stokes \cite{EdwardsTuckermanFriesnerEtAl1994}.
