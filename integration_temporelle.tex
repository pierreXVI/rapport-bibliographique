...
\section{Analyse des méthodes}
  ...
  \section{Consistance et ordre}
    ...
    \GP{De plus, on dira que la méthode est $p$-exacte, c'est-à-dire capable de représenter un polynôme de degré $p$}, si l'erreur locale est en $\Delta t^{p+1}$ \cite{HairerNorsettWanner1993} :
    \footnote{\GP{Cette phrase remplace l'initiale car je trouve que c'est plus clair...}}
