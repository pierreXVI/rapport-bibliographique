...
\section{Analyse des méthodes}
  ...
  \section{Consistance et ordre}
    ...
    Pour définir cette notion, on se place dans le cadre de l'équation (\ref{eq:edo})\footnote{\GP{Astuce de vieux briscard : l'équation n'est pas dans le chapitre. Aide le lecteur et remet la STP.}}.
    ...
    \paragraph{}
    De plus, on dira que la méthode est d'ordre $p$ si l'erreur locale\footnote{\GP{J'ai un doute... Ordre p+1 pour moi...}} est en $\Delta t^{p+1}$ :
...
\section{Méthodes implicites}
  ...
  \GP{Opposé à ces méthodes, on trouve les méthodes implicites.
  ...
  \subsection{Méthodes de Runge-Kutta}
    ...
    \PS{citer Iserles, Arieh (1996), A First Course in the Numerical Analysis of Differential Equations ou enlever.}\GP{Oui, citer !}
    ...
    \subsubsection{DIRK}
      ...
      En pratique, c'est un gros avantage car on peut alors factoriser la matrice une seule fois pour l'inverser à chaque étape.\GP{Référence?}
      %GP : pas utile sauf référence...
    ...
    \subsubsection{Méthodes de Rosenbrock}
      ...
      En pratique, pour permettre plus de liberté dans le choix de la méthode de Rosenbrock, on remplacera le terme\footnote{\GP{Tu ne dis pas les conséquences mathématiques d'un tel changement}} $a_{ii}F'F_{n,i}$ par $F'\sum_{j=1}^i\gamma_jF_{n,j}$.
      La méthode est donc :
