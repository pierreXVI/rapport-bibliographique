...
\paragraph{}
Nous avons vu dans l'introduction que lorsqu'on résout un problème de simulation numérique de la dynamique des fluides, on se ramène à résoudre une équation différentielle ordinaire de la forme :\footnote{\LM{Euh, du coup $Ax=b$ c'est pas vraiment une EDO, tu anticipes un peu la résolution du système linéaire du à l'implicite, non?}}
...
\section{Analyse des méthodes}
  ...
  \section{Consistance et ordre}
    ...
    \footnote{\GP{Cette phrase remplace l'initiale car je trouve que c'est plus clair...}\PS{Guillaume propose "\GP{De plus, on dira que la méthode est $p$-exacte, c'est-à-dire capable de représenter un polynôme de degré $p$}" mais j'aimerai bien laisser le terme "ordre" qui est plus précis, c'est le terme employé dans la littérature. Je propose donc de rajouter les deux phrases d'après.}}
  ...
  \section{Stabilité}
    ...
    \subsubsection{Méthodes mono-pas}
      ...
      \paragraph{}
      Il découle de l'équation précédente que si $G\left(\Delta t\alpha_i\right)^n$\footnote{\LM{tend vers 0, ou $\vert G\left(\Delta t \alpha_i\right)\vert < 1$}}, alors $y_n$ tend vers 0.
      ...
      On comprend alors tout de suite que pour que l'algorithme soit stable, il faut que les valeurs propres de $J$ soient toutes dans le domaine de stabilité de la méthode.\footnote{\LM{Je suis un peu pointilleux peut-être, mais il me semble que ces deux phrases abordent deux aspects assez différents; il y a tout d'abord, et indépendamment de la méthode d'intégration, une propriété intrinsèque de l'équation linéarisée qui rend la solution $\tilde{W}$ "attractive" (il me manque le terme exact, Guillaume complétera), propriété qui peut se traduire par les parties entières négatives des valeurs propres de J. Et d'autre part, il y a la capacité de la méthode à préserver cette "attractivité" et d'éviter que son imperfection ne finisse par faire diverger $y$ et donc mener la solution reconstruite dans les choux. C'est pourquoi j'aurais tendance à aborder le premier point avant la subsubsection "monopas"  }}
...
\section{Méthodes implicites}
  ...
  \subsection{Méthodes de Runge-Kutta}
    ...
    \subsubsection{Méthodes de Rosenbrock}
      ...
      \footnote{\GP{Tu ne dis pas les conséquences mathématiques d'un tel changement}\PS{Pas non plus dans le texte de référence, je peux cependant remonter à la source pour trouver ça, mais est-ce que c'est pas un peu m'éloigner du sujet ?}\GP{Si Lionel OK, on laisse comme cela...}\LM{Ben, excusez ma naïveté, je ne connais pas bien les subtilités de la méthode, mais, de loin, dans le brouillard, on a l'impression qu'on est pas loin d'avoir simplement linéarisé autour de $W_n$... Il y a des versions de la méthode dans lesquelles on franchit le pas et on pose $a_{ij} \equiv 0$ pour tout faire porter sur les $\gamma_{ij}$?}}
      La méthode est donc :
      \begin{equation}\label{eq:rk_rosenbrock}
        \left\{\begin{aligned}
          W_{n+1} &= W_n + \Delta t\sum_{i = 1}^kb_iF_{n,i} \\
          \;\textrm{avec}\quad F_{n,i} &= F\left(W_n + \Delta t\sum_{j = 1}^{i-1}a_{ij}F_{n,j}\right) + \Delta tF'\sum_{j=1}^i\gamma_{ij}F_{n,j}
        \end{aligned}\right.\ .
      \end{equation}
...
\section{Étapes du calcul}
  ...
  Par exemple, pour la méthode d'Euler implicite appliquée à (\ref{eq:edo}), on cherche à chaque itération le zéro de $\delta W \mapsto \delta W - \Delta t F\left(W_n + \delta W\right)$.
