...
\section{Analyse des méthodes}
  ...
  \section{Stabilité}
    ...
    \subsubsection{Méthodes multi-pas}
      ...
      On cherche ensuite les $y_{n+1-i}$ sous la forme $\mu^{k-i}$ et on identifie alors le polynôme $G_z\left(\mu\right) = \mu^k - \sum_{i=0}^{k-1}g_i\left(z\right)\mu^i$.
...
\section{Méthodes explicites}
  ...
  \subsection{Méthodes d'Adams-Bashforth}
    ...
    Cependant, on constate que la stabilité de ces méthode décroît avec l'ordre, et qu'elles ne sont pas adaptées pour les problèmes raides que l'on cherche à résoudre.
...
\section{Méthodes implicites}
  ...
  \paragraph{}
  Comme pour les méthodes d'Adams-Bashforth et les autres méthodes multi-pas, l'analyse de stabilité est un peu plus complexe qu'avec les méthodes mono-pas.
  % Cependant, les domaines de stabilité des méthodes BDF sont satisfaisants, comme on peut le voir sur la figure \ref{fig:bdf_stab}.
  %
  % \begin{figure}
  %   \centering
  %   \includegraphics{images/bdf_stab.png}
  %   \caption{Domaines de stabilité des méthodes BDF.}
  %   \label{fig:bdf_stab}
  % \end{figure}
