...
\section{Analyse des méthodes}
  ...
  \section{Stabilité}
    ...
    \subsubsection{Méthodes mono-pas}
      ...
      \footnote{\LM{Je suis un peu pointilleux peut-être, mais il me semble que ces deux phrases abordent deux aspects assez différents; il y a tout d'abord, et indépendamment de la méthode d'intégration, une propriété intrinsèque de l'équation linéarisée qui rend la solution $\tilde{W}$ "attractive" (il me manque le terme exact, Guillaume complétera), propriété qui peut se traduire par les parties entières négatives des valeurs propres de J. Et d'autre part, il y a la capacité de la méthode à préserver cette "attractivité" et d'éviter que son imperfection ne finisse par faire diverger $y$ et donc mener la solution reconstruite dans les choux. C'est pourquoi j'aurais tendance à aborder le premier point avant la subsubsection "monopas"  }
      \PS{Je ne suis pas sur de comprendre ce que tu dis : la raison pour laquelle on linéarise n'est pas du tout pour qu'on résolve nous en CFD l'équation, c'est pour que les gens des mathématiques appliquées fassent leur analyse de stabilité. Dans cet exemple J est quelconque, c'est pas la jacobienne d'un vrai flux. L'équation linéarisée c'est juste pour expliquer pourquoi on regarde la stabilité sur l'équation de Dahlquist.}
      }
...
\section{Méthodes implicites}
  ...
  \subsection{Méthodes de Runge-Kutta}
    ...
    \subsubsection{Méthodes de Rosenbrock}
      ...
      \PS{Ah après libre à toi de choisir tes coefficients comme tu le souhaites, il y a des conditions à respecter pour garantir l'ordre mais c'est tout. Là j'ai juste voulu mettre la définition "générale".}}
      La méthode est donc :
