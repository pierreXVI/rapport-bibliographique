...
\section{Méthodes de Krylov}
  ...
  \subsection{Principe des méthodes}
    ...
    \paragraph{}
    Les méthodes de Krylov \GP{semblent} s'imposer comme étant la norme dans le domaine de la dynamique des fluides numérique.
...
\section{Améliorations de l’algorithme}
    ...
    \end{equation}
		...
		C'est parce que puisque la matrice est non-normale, le conditionnement seul ne \GP{suffit pas à comprendre ce qui se passe}%veut rien dire
    ...
    \footnote{\PS{Dois-je montrer que GMRES n'est pas linéaire ? Ça ne sort pas d'une référence, mais j'ai la preuve sauf que c'est un peu long. En fait j'ai retrouvé dans mes dossiers à l'ONERA la preuve déjà rédigée en LaTeX}\GP{Perso, je pense que c'est cool de mettre la preuve, mais sans doute en annexe. Qu'en pensez-vous ?}}
  ...
  \subsection{Recyclage de l'information}
    ...
    \GP{On peut par exemple utiliser de telles méthodes de recyclage non pas entre les cycles d'une résolution GMRES, mais pourquoi au cours d'une résolution successive de plusieurs problèmes linéaires \cite{Gaul2014}, ce qui est précisément ce que l'on fait dans notre cas\footnote{phrase bizarre}.}
