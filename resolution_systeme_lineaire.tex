...
\section{Améliorations de l’algorithme}
	...
	\subsection{Préconditionnement}
		...
		quand on dispose d'une factorisation de $A$ (factorisation LU : $A = LU$, ou de Cholesky quand $A$ est hermitienne : $A = L\adj{L}$), et que la matrice $A$ est creuse, les matrices intervenant dans ces décomposition perdent la creusité.
		On va donc garder ces matrices en leur imposant le pattern de creusité de $A$ :
		\begin{itemize}
			\item $M=\widetilde{L}\widetilde{U}$ avec $\widetilde{L}_{ij}=L_{ij}$ et $\widetilde{U}_{ij}=U_{ij}$ si $A_{ij}\neq0$ et $\widetilde{L}_{ij}=\widetilde{U}_{ij}=0$ sinon, pour la décomposition LU,
			\item $M=\widetilde{L}\adj{\widetilde{L}}$ avec $\widetilde{L}_{ij}=L_{ij}$ si $A_{ij}\neq0$ et 0 sinon, pour la décomposition de Cholesky.
		\end{itemize}
		On peut généraliser cette factorisation incomplète en prenant le pattern de creusité de $A^k$, plus dense que le pattern de creusité de $A$ mais globalement creux.
		...
		l'idée est d'effectuer un (ou quelques) pas d'une méthode itérative classique.
		...
		L'idée est donc de prendre $M_{pre} = M^{-1}N$.
	...
	\subsection{Préconditionnement flexible}
		...
		Son principe est plutôt simple : il consiste à réaliser
