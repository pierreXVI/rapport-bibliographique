...
\section{Introduction}
	...
	\subsection{Contexte}
		...
		Les entreprises comme les laboratoires de recherche ont besoin de pouvoir accéder à certaines grandeurs physiques associées à des phénomènes et à des régimes de fonctionnement \GP{qui ne sont pas possible de réaliser à notre échelle, la limitation pouvant être matérielle ou bien financière}\footnote{\GP{Pas super bien dit...}}.
		...
		Par travail, on peut entendre ici une puissance de calcul multipliée par un temps de calcul : n'importe quelle machine ne peut pas réaliser n'importe quelle simulation, certains cas nécessitent des machines très puissantes, et même avec ces machines la simulation numérique n'est pas instantanée, et il est courant de lancer des calculs durant plusieurs 
		semaines\footnote{\GP{Les simulations turbulentes à grand nombre de Reynolds sont inaccessibles...}}.
	...
	\subsection{Le problème stationnaire}
		...
		\GP{Il y a un peu de redite... Cela rend le paragraphe pas aisé à suivre...}
		...
		\paragraph{}
		\GP{Puisqu'on cherche une solution stationnaire, on l'exprime comme un zéro de cette fonction.}\\
		\GP{Pour obtenir cette solution, on introduit la notion de pseudo-temps \cite{KelleyKeyes1996}.}
		...
		Inversement, utiliser un algorithme tel que la méthode de Newton de manière brute sur la fonction $\operatorname{f}$ n'aboutit pas\footnote{\GP{C'est toujours vrai???}}.
	...
	\subsection{Discrétisation}
		...
		\paragraph{}
		Lorsque l'on résout une équation aux dérivées partielles avec une méthode numérique, il est nécessaire de réaliser une discrétisation du problème, ne serait-ce que pour pouvoir représenter l'état du système dans un domaine \GP{borné 
		représentable dans la mémoire d'}un ordinateur.
...
\section{Résolution du système linéaire}
	...
	\subsection{Méthodes de Krylov}
		...
		On itère ensuite sur $n$ : en connaissant $V_n$, la relation (\ref{eq:arnoldi_n+1}) permet de trouver le nouveau vecteur de base et une nouvelle colonne de $H$, c'est à dire de trouver $V_{n+1}$ et $\widetilde{H}_n$.
